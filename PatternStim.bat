@echo off
title PatternStim - patterned OSS stimulation
...
call "%CONDA_PATH%"
conda env list | findstr /B /C:"patternstim " >nul
if errorlevel 1 (
    echo Environnement 'patternstim' introuvable : creation depuis environment.yml (quelques minutes)...
...
call conda activate patternstim

:: 5. Lancement DETACHE et silencieux (pythonw = pas de console noire)
start "" pythonw -m patternstim gui
